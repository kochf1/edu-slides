...
\pgfkeys{
    ...
    %
        % Texts
        text/locations/.initial   = {., ./images}, % Folder locations to look for Texts
    %
        % Images
...
\begin{document}
    ...
    \slideImageText{slideImageText}{img-1}{!text-1.txt}
    ...
    \slideQuoteText{The centrality of interactions}{!text-1.txt}
    \slideTextQuote{The centrality of interactions}{!text-1.txt}
    \slideQA{Could this really work?}{A: It all depends}
    \slideLiterature{book-cover}{http://somewhere.com/book}{Title}{!book-chapters.txt}

    %% Explore Concept templates
    % Concepts with Text & Explanation
    \slideConcept[title=slideConcept, font=big, one=72\%]{!text-1}
    \slideConceptTwo[title=slideConceptTwo, font=big, color=true, one=72\%, two=25\%]{!text-1}{!text-1.txt}
    ...
    \slideConcept[title=slideConcept (image), type=image, one=fau-logo]{!text-1}
